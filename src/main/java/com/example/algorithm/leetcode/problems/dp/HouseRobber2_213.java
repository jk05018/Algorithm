...
public class HouseRobber2_213 {
	...
	public int rob(int[] nums) {
		...
		return Math.max(getRobbedMoney(nums, 0, nums.length - 1), getRobbedMoney(nums, 1, nums.length));
	}

	public int getRobbedMoney(int[] nums, int start, int end) {
		int rob1 = 0;
		int rob2 = 0;
	...
	*/
	public int rob1(int[] nums) {
		...
		return Math.max(findMaxWealthNotTakingFirstOrLast(nums, 1, length - 1),
			findMaxWealthNotTakingFirstOrLast(nums, 0, length - 2));
	}
	...
	*/
	...
	public int rob2(int[] nums) {
		...
		// the first option is do not consider the first element, but the last element
		// the second option is we consider first element, but not the last element.
		return Math.max(findMaxWealthNotTakingFirstOrLast(nums, 1, length - 1, dp1),
			findMaxWealthNotTakingFirstOrLast(nums, 0, length - 2, dp2));
	}
	...
	*/
	...
	public int rob3(int[] nums) {
		...
		int[] dp2 = new int[length];
		Arrays.fill(dp2, -1);
		return Math.max(maxResultLeavingNotLeavingFirstLast(arr1, dp1), maxResultLeavingNotLeavingFirstLast(arr2, dp2));
	}
